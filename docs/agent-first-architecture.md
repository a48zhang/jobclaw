...
- `targets.md` / `userinfo.md` 等工作区文档由主 Agent 在对话中持续维护，配置页只作为人工校对和覆写界面
...
- `targets.md` / `userinfo.md` 虽然是 Markdown，但它们应被视为 Agent 与用户共享的工作区记忆，而不是要求用户先手工完成的固定表单。

### 4.6 用户问题是升级点，不是默认前置

后续产品不应再用“先补完资料再开始”的方式阻塞主路径。

应坚持：

- 能从聊天、历史文档和已有状态中推断出的信息，优先由主 Agent 起草
- 能先做广义搜索或低风险执行的场景，优先先执行再回补细化
- 只有在缺失信息会显著改变决策或引入风险时，再追问用户
