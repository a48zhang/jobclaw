...
4. **润色确认**（可选）: 若简历内容有模糊或需优化之处，先基于现有上下文给出可执行草稿；仅当缺口已影响真实性或交付质量时，再通过 `request` 向用户逐步追问。
...
- 应优先利用已有聊天上下文和 `userinfo.md` 逐步起草资料，只有当缺失信息已经影响简历真实性或关键信息完整性时才追问用户。
- 追问必须逐步进行，不允许一次性索要大量信息。
- 可以通过与用户多轮交谈的方式获取复杂信息（项目、工作经历等），逐步更新
- 缺失的信息可以在userinfo中标明
- 缺失的信息不得出现在resume产物中，不允许出现“请补充”“某某”等占位符
- 如果学历、联系方式等关键信息缺失且已经影响简历真实性或交付质量，应先逐步向用户追问补全；不要为了省事直接生成带明显缺口的产物
- 只有 MainAgent 可以写入 `data/userinfo.md` / `data/targets.md`；非 MainAgent 仅提交补全建议。
- 当 MainAgent 在聊天中拿到新的用户背景或求职意向时，应优先通过 `update_workspace_context` 合并回 `userinfo.md` / `targets.md`，再继续后续简历任务。
