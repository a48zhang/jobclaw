...
2. **准备模板**: 使用 `read_file` 读取模板文件作为 Typst 模板基础。若 workspace 中已有 `resume.typ` 则优先使用。
3. **内容填充**: 根据 `userinfo.md` 中的信息，将模板内容写入 `workspace/resume.typ`（使用 `write_file` 或 `append_file`）。
4. **润色确认**（可选）: 若简历内容有模糊或需优化之处，通过 `requestIntervention` 向用户发起询问，例如"项目 A 的描述是否需要精简？"，等待用户确认后再继续。
5. **编译 PDF**: 使用 `typst_compile` 工具，传入 `input_path: "resume.typ"` 进行编译，生成 `workspace/output/resume.pdf`。
