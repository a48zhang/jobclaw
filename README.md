...
- **自动化投递**：专用的 `DeliveryAgent` 负责执行复杂的表单填写与投递操作，自主导航各类招聘门户。
- **人工干预 (HITL)**：在涉及多因子认证 (MFA) 或简历最终确认等关键环节，通过 Web 或 TUI 实时请求用户介入。
- **实时监控看板**：
...
2. **DeliveryAgent (投递智能体)**：临时专家。由主智能体派生，专注于执行高精度的浏览器自动化操作。
...
- **LLM**: 兼容 OpenAI 协议的 API (建议使用 GPT-4o, o3-mini 等)
